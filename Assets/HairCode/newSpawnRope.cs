...
public class newSpawnRope : MonoBehaviour
{
    ...
    public void Spawn()
    {
        ...
        { 
            ...
            if (i == 0)
            {
                Destroy(tmp.GetComponent<SpringJoint>());
                if (snapFirst)
                {
            ...
            {
                tmp.GetComponent<SpringJoint>().connectedBody = parentObject.transform.Find((parentObject.transform.childCount - 1).ToString()).GetComponent<Rigidbody>();
